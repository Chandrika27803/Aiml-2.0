...
- Opens a Tkinter window with:
  - Tab 1: 2D graphs (scatter, line, hist, box, cat vs num, time series)
  - Tab 2: 3D graphs (numeric 3D, time+2 numeric)
  - Tab 3: Plot Suggestions (text)

- Closing the window exits the program normally.
"""
...
def install_sigint_handler(root: tk.Tk):
    """
    Make Ctrl+C in the terminal close the Tk window and exit the program.
    """
    ...
    # On Windows this still works for Ctrl+C in the console
...
def build_plot_suggestions_text(
    ...
    so the Suggestions tab matches the graphs.
    """
    lines = []
...
def open_dashboard(df_clean, X_numeric, y,
    ...
    """
    Build the Tkinter dashboard window with tabbed 2D/3D graphs + suggestions.
    ...
    # This is the signal handler that will exit cleanly
    install_sigint_handler(root)
    ...
    notebook.add(tab_sugg, text="Plot Suggestions")
    ...
    # Default WM_DELETE_WINDOW behaviour: destroy root → mainloop returns → program ends
    root.mainloop()
...
def main():
    ...
    # Build and print suggestions (also used in Suggestions tab)
    ...
    # 9. Open the GUI dashboard with tabs for 2D, 3D, and suggestions
