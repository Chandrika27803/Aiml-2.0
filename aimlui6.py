...
- Auto:
  ...
  - Trains multiple models on a cleaned subset (drops NaN only where needed):
      * Decision Tree
      * Random Forest
      * Logistic Regression (for classification) / Linear Regression (for regression)
  - Prints performance metrics for Decision Tree (primary demo model)
...
- Opens a Tkinter window with:
  - Tab 1: ML Summary (model comparison, feature importance)
  - Tab 2: Possible Graphs (text suggestions)
  - Tab 3: 2D graphs (scatter, line, hist, box, cat vs num, time series)
  - Tab 4: 3D graphs (numeric 3D, time+2 numeric)
...
def analyze_columns(df: pd.DataFrame):
    ...
    for col in df.columns:
        ...
        if series.dtype == "object":
            # Coerce errors to NaT; if most values parse, treat as datetime
...
def create_box_figure(X):
    ...
    ax.boxplot(
        ...
        tick_labels=numeric_cols,  # Matplotlib 3.9+
        vert=True
    )
...
def open_dashboard(df_clean, X_numeric, y,
    ...
    ml_text = tk.Text(tab_ml, wrap="word")
    ml_text.pack(fill="both", expand=True, padx=5, pady=5)

    if ml_results is None or not ml_results.get("models"):
        ml_text.insert(
            "1.0",
            "Machine learning was not run.\n\n"
            "Possible reasons:\n"
            "- Not enough fully usable rows for ML, or\n"
            "- Target had too many classes for this simple demo.\n\n"
            "Graphs and column analysis are still available in other tabs."
        )
        ml_text.config(state="disabled")
    else:
        # Create a small top frame for combo
        top_frame = ttk.Frame(tab_ml)
        top_frame.pack(fill="x", padx=5, pady=5)

        ttk.Label(
            top_frame,
            text=f"Problem type: {ml_results['problem_type']}    "
                 f"Target: {ml_results['target_col']}"
        ).pack(side="left", padx=5)

        ttk.Label(top_frame, text="Model:").pack(side="left", padx=(20, 5))

        model_names = list(ml_results["models"].keys())
        combo = ttk.Combobox(top_frame, values=model_names, state="readonly")
        combo.current(0)
        combo.pack(side="left")

        def render_model(name: str):
            ml_text.config(state="normal")
            ml_text.delete("1.0", "end")

            info = ml_results["models"][name]
            metric_name = info["metric_name"]
            metric_val = info["metric_value"]
            y_true = info.get("y_test_sample", [])
            y_pred = info.get("y_pred_sample", [])
            feat_imp = info.get("feature_importances", [])

            lines = []
            lines.append(f"Problem type : {ml_results['problem_type']}")
            lines.append(f"Target column: {ml_results['target_col']}")
            lines.append(f"Model        : {name}")
            lines.append(f"{metric_name}: {metric_val:.4f}")
            lines.append("")

            if y_true and y_pred:
                lines.append("Sample predictions vs actual (first 10):")
                for i, (yp, yt) in enumerate(zip(y_pred, y_true)):
                    lines.append(f"  {i+1:2d}. pred = {yp}   actual = {yt}")
                lines.append("")

            if feat_imp:
                # sort by absolute importance descending
                feat_imp_sorted = sorted(
                    feat_imp, key=lambda t: abs(t[1]), reverse=True
                )[:15]
                lines.append("Feature importance / coefficients (top 15):")
                max_name_len = max(len(f) for f, _ in feat_imp_sorted)
                for f, v in feat_imp_sorted:
                    lines.append(f"  {f:<{max_name_len}} : {v:+.4f}")
            else:
                lines.append("No feature importance available for this model.")
...
def main():
    ...
    ml_results = None  # default if ML is skipped
    ...
    if df_clean_for_ml.shape[0] >= MIN_ML_ROWS:
        ...
        if is_classification and n_unique > MAX_CLASSES_FOR_DEMO:
            ...
            # ---- Decision Tree (primary demo model) ----
            if is_classification:
                dt = DecisionTreeClassifier(random_state=42)
                dt.fit(X_train, y_train)
                y_pred_dt = dt.predict(X_test)
                metric_dt = accuracy_score(y_test, y_pred_dt)
                metric_name = "Accuracy"
            else:
                dt = DecisionTreeRegressor(random_state=42)
                dt.fit(X_train, y_train)
                y_pred_dt = dt.predict(X_test)
                metric_dt = mean_squared_error(y_test, y_pred_dt)
                metric_name = "MSE"

            print("\n[Decision Tree] Sample predictions vs actual (first 10):")
            print("Predictions:", list(y_pred_dt[:10]))
            print("Actual     :", list(y_test[:10].values))
            if is_classification:
                print(f"\n[Decision Tree] Accuracy: {metric_dt * 100:.2f}%")
            else:
                print(f"\n[Decision Tree] MSE: {metric_dt:.4f}")

            feat_imp_dt = []
            if hasattr(dt, "feature_importances_"):
                for fname, val in zip(X_numeric_ml.columns, dt.feature_importances_):
                    feat_imp_dt.append((fname, float(val)))

            models_info["Decision Tree"] = {
                "metric_name": metric_name,
                "metric_value": float(metric_dt),
                "y_test_sample": list(y_test[:10].values),
                "y_pred_sample": list(y_pred_dt[:10]),
                "feature_importances": feat_imp_dt,
            }
            ...
            if is_classification:
                ...
                metric_rf = accuracy_score(y_test, y_pred_rf)
                metric_name_rf = "Accuracy"
            else:
                ...
                rf.fit(X_train, y_train)
                y_pred_rf = rf.predict(X_test)
                metric_rf = mean_squared_error(y_test, y_pred_rf)
                metric_name_rf = "MSE"

            feat_imp_rf = []
            if hasattr(rf, "feature_importances_"):
                for fname, val in zip(X_numeric_ml.columns, rf.feature_importances_):
                    feat_imp_rf.append((fname, float(val)))

            models_info["Random Forest"] = {
                "metric_name": metric_name_rf,
                "metric_value": float(metric_rf),
                "y_test_sample": list(y_test[:10].values),
                "y_pred_sample": list(y_pred_rf[:10]),
                "feature_importances": feat_imp_rf,
            }
            ...
            if is_classification:
                ...
                try:
                    ...
                    metric_lr = accuracy_score(y_test, y_pred_lr)
                    metric_name_lr = "Accuracy"

                    # coefficients: average absolute across classes
                    coef = lr.coef_
                    if coef.ndim == 2:
                        ...
                        coef_mean = coef
                    feat_imp_lr = [
                        (fname, float(v)) for fname, v in zip(X_numeric_ml.columns, coef_mean)
                    ]

                    models_info["Logistic Regression"] = {
                        "metric_name": metric_name_lr,
                        "metric_value": float(metric_lr),
                        "y_test_sample": list(y_test[:10].values),
                        "y_pred_sample": list(y_pred_lr[:10]),
                        "feature_importances": feat_imp_lr,
                    }
                except Exception as e:
                    print("\n[Logistic Regression] Skipped due to error:", e)
            else:
                lr = LinearRegression()
                lr.fit(X_train, y_train)
                y_pred_lr = lr.predict(X_test)
                metric_lr = mean_squared_error(y_test, y_pred_lr)
                metric_name_lr = "MSE"

                coef = lr.coef_
                # coef is 1D for standard regression
                feat_imp_lr = [
                    (fname, float(v)) for fname, v in zip(X_numeric_ml.columns, coef)
                ]
    ...
    # 8. Open the GUI dashboard with tabs for ML Summary, Possible Graphs, 2D, 3D
    open_dashboard(df_for_analysis, X_numeric_plot, y_full,
                   numeric_cols_all, categorical_cols, datetime_cols,
                   suggestions_text,
